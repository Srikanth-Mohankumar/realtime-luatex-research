...
rewrites a section. The fix is not a faster compiler --- \LuaTeX{}'s
line breaker needs microseconds per paragraph~\cite{lode:realtime} ---
but an architecture that stops discarding state between builds.
...
\begin{figure*}[tb]
\centering
\resizebox{\textwidth}{!}{\input{figs/pgc-architecture.tikz}}
...
\begin{figure}[tb]
\centering
\resizebox{\columnwidth}{!}{\input{figs/pgc-protocol.tikz}}
...
points in \LuaTeX's typesetting pipeline:
...
\begin{figure*}[tb]
\centering
\resizebox{\textwidth}{!}{\input{figs/pgc-keystroke.tikz}}
...
renders glyphs by character code with the viewer's fonts, which is a
deliberate approximation: positions are engine-exact, faces are
approximate, and the next build replaces the overlay with truth.
